...
public interface HospitalRepository extends JpaRepository<Hospital, Long> {
	...
	@Query("SELECT COUNT(h.id) FROM Hospital h WHERE h.name = :username AND h.employeeTel = :employeeTel")
	int findCountByNameAndEmployeeTel(@Param("username") String username, @Param("employeeTel") String employeeTel);
