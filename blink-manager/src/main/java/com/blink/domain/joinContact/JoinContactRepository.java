...
public interface JoinContactRepository extends JpaRepository<JoinContact, Long> {
	...
	@Query("SELECT new com.blink.web.admin.web.dto.joinConcact.SingleJoinConcactResponseDto(c.id, c.clinicName, c.regDate, c.name, c.tel, c.answerYn, c.inquiry, c.usedProgram, c.email, c.answerContent, c.visitReserveYn, c.visitDate, c.visitAim) FROM JoinContact c WHERE (c.clinicName LIKE %:searchText% OR c.usedProgram LIKE %:searchText%) AND c.regDate > :time")
