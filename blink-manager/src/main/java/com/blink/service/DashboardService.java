...
public class DashboardService {
	...
	public DashboardResponseDto getDashboardData(String searchText, Date startDate, Date endDate, Pageable pageable) {

		Page<DashboardHospitalResponseDto> dailyHospital = statisticsDailyHospitalReporitory
				.findBySearchTextAndPeriod(searchText, startDate, endDate, pageable);

		Integer totalExaminationCount = statisticsDailyHospitalReporitory.findTotalExaminationCount(startDate, endDate);
		Integer totalExamineeCount = statisticsDailyHospitalReporitory.findTotalExamineeCount(startDate, endDate);
		Integer totalAgreeYCount = statisticsDailyHospitalReporitory.findTotalAgreeYCount(startDate, endDate);
		Integer totalAgreeNCount = statisticsDailyHospitalReporitory.findTotalAgreeNCount(startDate, endDate);
		Integer nonexistConsetFormCount = metadataRepository.findNonexistConsetFormCountForAdmin(startDate, endDate);
		DashboardResponseDto responseDto = new DashboardResponseDto(totalExaminationCount, totalExamineeCount,
				totalAgreeYCount, totalAgreeNCount, nonexistConsetFormCount, dailyHospital);

		return responseDto;
	}

	public Map<String, Object> getDashboardDataWithHospital(Long hospitalId, Integer year, Integer month,
			Pageable pageable) {
		...
		// List<PieChartResult> ageGrChartResults =
		// statisticsService.findAgeGroupChartResult(hospitalName);
		// result.put("ageGroupChartResult", ageGrChartResults);
		//
		// List<PieChartResult> genderGroupChartResult =
		// statisticsService.findGenderGroupChartResult(hospitalName);
		// result.put("genderGroupChartResult", genderGroupChartResult);
		//
		// List<PieChartResult> inspectionTypeGroupChartResult =
		// statisticsService.findInspectionTypeGroupChartResult(hospitalName);
		...
		for (int i = 1; i <= 7; i++) {
			...
			Integer examineeCount = statisticsDailyHospitalReporitory.findExamineeCountByHospitalAndDayOfWeek(hospital,
					year, i);
		...
		for (int i = 1; i <= 4; i++) {
			...
			Integer examineeCount = statisticsDailyHospitalReporitory.findExamineeCountByHospitalAndMonth(hospital,
		...
		Integer yearlyConsentFormCount = consentFormRepository.findYearlyConsentFormCount(hospital,
				String.valueOf(year));
